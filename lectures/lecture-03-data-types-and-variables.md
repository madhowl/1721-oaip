# Лекция 3: Типы данных и переменные

## Введение

В предыдущей лекции мы познакомились с языком Python и его основными особенностями. Теперь мы подробнее рассмотрим, как в Python работают переменные и типы данных. Понимание этих концепций является фундаментом для написания любых программ на Python. В этой лекции мы изучим основные типы данных, как создавать переменные, как с ними работать и как преобразовывать типы данных.

## Основное содержание

### Переменные в Python

Переменная — это имя, которое связано со значением в памяти компьютера. В Python создание переменной происходит при присваивании значения с помощью оператора `=`. В отличие от многих других языков программирования, в Python не нужно объявлять тип переменной заранее — тип определяется автоматически при присваивании значения.
...
Python имеет несколько встроенных типов данных, которые можно разделить на несколько категорий:
...
В Python существуют правила именования переменных:

1. Имена переменных могут содержать буквы (a-z, A-Z), цифры (0-9) и символ подчеркивания (_)
2. Имя не может начинаться с цифры
3. Имена чувствительны к регистру (age и Age — разные переменные)
4. Нельзя использовать ключевые слова Python как имена переменных
...
student_name = "Иван"
age2 = 25
_private_var = "Это частная переменная"
```
...
print(f"Налог ({tax_rate*100}%): {tax_amount:.2f} руб.")
print(f"На руки: {net_salary:.2f} руб.")
```

## Заключение

В этой лекции мы подробно рассмотрели основные типы данных в Python:
...
Правильное использование типов данных и переменных позволяет писать более понятный и эффективный код. В следующей лекции мы рассмотрим условные конструкции, которые позволят нашим программам принимать решения на основе различных условий.
